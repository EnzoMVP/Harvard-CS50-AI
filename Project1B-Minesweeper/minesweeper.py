    ...
    def add_knowledge(self, cell, count):
        ...
        for i in [-1, 0, 1]:
            for k in  [-1, 0, 1]:
                if i == 0 and k == 0:
                    continue
                if 0 <= row + i < self.height and 0 <= column + k < self.width:
                    neighbors.add((row+i, column+k))
        ...
        while True:
            ...
            for sent in self.knowledge:
                ...
                if safe_cells:
                    for safe_cell in list(safe_cells):
            ...
            for i in range(len(self.knowledge)):
                for j in range(i+1, len(self.knowledge)):
                    cells1 = self.knowledge[i].cells
                    count1 = self.knowledge[i].count
                    cells2 = self.knowledge[j].cells
        ...
        for i in range(self.height):
            for j in range(self.width):
                if (i, j) not in self.mines and (i, j) not in self.moves_made:
                    not_selected.append((i, j))
        if not_selected:
            return random.choice(not_selected)
        return None